...
​        --->  s[i] == ')'  ---> s[i-1] == '(', 形如....(), 此时后面这两个括号一定是成对的，最大长度= 2+dp[i-2]
                            ---> s[i-1] == ')', 形如....)), 此时根本不知道这两个右括号和谁是成对的，利用dp[i-1]信息
                                 假设以s[i-1]结尾的最长有效括号长度为n且不为0，(n==0就不用讨论了,dp[i]=0)则与i-1位置成对出现的左括号的下标为i-n, 再看下标i-n-1处是有可能和i下标处成对出现的
                                            则. .  x     (   ...   )   )
                                                 i-n-1  i-n       i-1  i
...
2、循环更新：l和r两个指针初始为0， 让r从0开始底层一直到长度n-1结束，每次r右扩时候看当前从S[L:R]闭区间上的子串是否满足无重复性，若不满足则L向右前进缩小范围直到满足无重复性这个条件，R-L+1为当前以s[r]结尾的最长子串
...
定义：dp[i] [j] 为s1从头开始的长度i和长度为s2的子串的最小编辑距离，所以维度为m+1*n+1

初始条件：i=0时，第一行为0,1,2，....n

递推公式：

   if s1[i-1] == s2[j-1] 相等时候肯定取这个字符，dp[i[]j] = dp[i-1] [ j-1]

 else 否则就从增、删、替换三者中取最小值



## 11、编辑距离打印变换步骤

除了dp数组，再构建一个path数组，path[i] [j] 的含义是当前的i,j格子是从哪里来的， 把从哪里来转换为操作
